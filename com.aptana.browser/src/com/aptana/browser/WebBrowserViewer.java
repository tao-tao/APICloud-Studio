...
public class WebBrowserViewer extends Composite {
	...
	protected void createCommandBar(Composite parent)
	{
		...
		BrowserSizeCategory[] categories = BrowserPlugin.getDefault().getBrowserConfigurationManager().getSizeCategories();
			...
			for (final BrowserSize size : sizes) {
				...
				menuManager.add(new Action(size.getName()) {
					...
					public void run() {
						...
						Color background = blackBackground ? getDisplay().getSystemColor(SWT.COLOR_BLACK) : getDisplay().getSystemColor(SWT.COLOR_WHITE);
		...
		menuManager.add(new Action(Messages.WebBrowserViewer_LBL_Server) {
			public void run() {
			}
		});

		ToolBarManager toolBarManager = new ToolBarManager(SWT.FLAT);
		Action action = new Action(Messages.WebBrowserViewer_LBL_Set,
				IAction.AS_DROP_DOWN_MENU) //$NON-NLS-1$
		{
	...
	protected void createActions() {
		consoleAction = new Action("openconsole") {
			{
//				setToolTipText("\u6253\u5F00\u63A7\u5236\u53F0");
				setToolTipText(Messages.WebBrowserViewer_TTP_Debug);
				setImageDescriptor(ImageResource.getImageDescriptor(ImageResource.IMG_ELCL_NAV_CONSOLE));
			}
		...
		refreshAction = new Action(Messages.WebBrowserViewer_TTP_Refresh) {
			{
	...
	private void updateNavigationButtons()
	{
		...
		consoleAction.setEnabled(browser.m_browser != null);
