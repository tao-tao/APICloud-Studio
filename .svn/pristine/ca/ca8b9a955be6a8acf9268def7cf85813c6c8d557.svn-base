...
import com.aptana.browser.WebBrowserViewer4Mac;
...
public class WebBrowserView extends ViewPart {
	...
	private WebBrowserViewer4Mac browserViewer4Mac;
	...
	public void createPartControl(Composite parent) {
		...
		if (PlatformUtil.isMac()) {
			browserViewer4Mac = new WebBrowserViewer4Mac(parent, WebBrowserViewer.NAVIGATION_BAR);
			WebKitBrowser4Mac browser = (WebKitBrowser4Mac) browserViewer4Mac.getBrowser();
			browser.addProgressListener(new ProgressListener() {
				public void changed(ProgressEvent event) {
					if (event.total == 0) {
						return;
					...
					if (event.current == 0) {
						...
						progressMonitor.done();
						progressMonitor
								.beginTask(StringUtil.EMPTY, event.total);
						progressWorked = 0;
					}
					if (progressWorked < event.current) {
						getStatusBarProgressMonitor().worked(
								event.current - progressWorked);
						progressWorked = event.current;
					}
				}
